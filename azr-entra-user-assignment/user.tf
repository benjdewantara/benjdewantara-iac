...
data "azuread_service_principal" "app_existing" {
  client_id = var.app_client_id
}

output "azuread_application_app_existing" {
  value = data.azuread_service_principal.app_existing
}
...
resource "azuread_app_role_assignment" "this" {
  app_role_id         = data.azuread_service_principal.app_existing.app_roles[0].id
  principal_object_id = data.azuread_user.user_existing.object_id
  resource_object_id  = data.azuread_service_principal.app_existing.object_id
}
