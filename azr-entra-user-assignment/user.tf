...
resource "azuread_service_principal" "user_existing" {
  client_id = data.azuread_user.user_existing.object_id
}
...
resource "azuread_app_role_assignment" "this" {
  app_role_id         = data.azuread_application.app_existing.app_roles[0].id
  principal_object_id = data.azuread_user.user_existing.object_id
  resource_object_id  = data.azuread_application.app_existing.object_id
}
